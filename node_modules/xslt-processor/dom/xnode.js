...
var XNode = /** @class */ (function () {
    ...
    XNode.prototype.setAttribute = function (name, value) {
        for (var i = 0; i < this.attributes.length; ++i) {
            if (this.attributes[i].nodeName == name) {
                this.attributes[i].nodeValue = "".concat(value);
                return;
            }
        }
    ...
    XNode.prototype.setTransformedAttribute = function (name, value) {
        for (var i = 0; i < this.transformedAttributes.length; ++i) {
            if (this.transformedAttributes[i].nodeName === name) {
                this.transformedAttributes[i].transformedNodeName = name;
                this.transformedAttributes[i].transformedNodeValue = "".concat(value);
                return;
            }
        }
        var newAttribute = XNode.create(constants_1.DOM_ATTRIBUTE_NODE, name, value, this);
    ...
    XNode.prototype.setAttributeNS = function (namespace, name, value) {
        for (var i = 0; i < this.attributes.length; ++i) {
            if (this.attributes[i].namespaceUri == namespace &&
                this.attributes[i].localName == this.qualifiedNameToParts("".concat(name))[1]) {
                this.attributes[i].nodeValue = "".concat(value);
                this.attributes[i].nodeName = "".concat(name);
                this.attributes[i].prefix = this.qualifiedNameToParts("".concat(name))[0];
                return;
            }
        }
        this.attributes.push(XNode.create(constants_1.DOM_ATTRIBUTE_NODE, name, value, this, namespace));
    };
